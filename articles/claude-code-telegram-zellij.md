...
![スマホからClaude Codeにリモート指示を出すコンセプト](/images/claude-code-telegram-zellij/hero.png)
...
![Channelsアーキテクチャ](/images/claude-code-telegram-zellij/architecture.png)
...
蓋閉じによるスリープを防ぐには `caffeinate` コマンドを使います（AC電源接続が必要）:

```bash
caffeinate -s &
```

外出前にこれを実行してから蓋を閉じます。ただしこのプロセスはターミナルを閉じたり再起動すると終了するため、再度実行が必要です。

:::message
**システム設定** → **バッテリー** → **オプション** → 「ディスプレイがオフのときに自動でスリープさせない」は、**アイドル時のスリープ防止**であり、**蓋閉じによるスリープは防げません**。蓋を閉じて運用する場合は `caffeinate` が必要です。
:::

:::message alert
**蓋閉じ運用のデメリット**
- **熱**: 蓋を閉じると排熱効率が落ちます。Channels待ち受け程度の軽い負荷なら問題ありませんが、重い処理が走ると熱がこもる可能性があります
- **バッテリー劣化**: AC電源に常時接続＝満充電状態が続くとバッテリーに負担がかかります。macOSの「バッテリー充電の最適化」を有効にしておけば80%付近で充電を止めてくれるため、影響を軽減できます
:::
...
![推奨ワークフロー](/images/claude-code-telegram-zellij/workflow.png)
